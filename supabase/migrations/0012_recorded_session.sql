...
begin
  ...
  v_clock := v_started + interval '2 minutes';

  for r in
    select * from (values
    ('ENG-DIAG-T4-M2-Q01', 'B', 65, 3, 'solid_reasoning', 'Started on "relative" from the options alone, then went back to the text and let "inherit their mother''s rank" and "compete" overturn it. That self-correction is the habit worth keeping.', 'First thought relative because I jumped to the options. Then I saw inherit their mother''s rank — you would compete to win something, so dominant.'),
    ('ENG-DIAG-T4-M2-Q02', 'D', 40, 3, 'solid_reasoning', 'Eliminated cleanly and got there without knowing "zealot" exactly. Worth writing the meanings down as we meet them.', 'Not A, not C. D, skeptics. Advocates makes no sense because you are not saying what you advocate for.'),
    ('ENG-DIAG-T4-M2-Q03', 'B', 35, 2, 'solid_reasoning', 'Picked B without knowing the word, purely by killing the other three. "As a result" plus "outspoken" plus "controversial" is what gives it away.', 'Not amateur, people came before her. Not a curiosity. I do not know what polemicist means but the others do not fit.'),
    ('ENG-DIAG-T4-M2-Q05', 'D', 95, 3, 'solid_reasoning', 'Named the elimination rule herself — extreme language is wrong — and applied it. Read both texts properly before answering.', 'A and B are the opposite of what is argued. C fails because self-care is not in text 1 and nature is not in text 2. D argues less strongly, which usually means it is right.'),
    ('ENG-DIAG-T4-M2-Q06', 'C', 60, 2, 'concept_gap', 'Main idea is the central message, not what the passage does. C is true of the poem — the spider does prompt reflection — but the point is the soul seeking connection. She reached D once we broke the metaphor down.', 'Speaker''s observations of a spider inspire them to reflect. Not B. I think C.'),
    ('ENG-DIAG-T4-M2-Q07', 'A', 55, 2, 'concept_gap', 'Detail questions want the paraphrase of the passage, not the one line that caught the eye. She anchored on "pushing away the memories" and missed that his whole character is being described.', 'It is not B, he is not overcome by grief. Suppress the past or warm disposition — I will say A.'),
    ('ENG-DIAG-T4-M2-Q08', 'D', 39, 3, 'solid_reasoning', 'Clean and fast. Spotted "unique specialty" and "shedding new light" as the signal for unconventional.', 'No policy, no strategy, nothing negative. Shedding new light implies not many people are in this field.'),
    ('ENG-DIAG-T4-M2-Q09', 'B', 53, 1, 'careless_error', 'She had C in hand — said it "could very well be C" — and stayed on B. Rushed the first read. Her instinct was right; the second-guess went the wrong way.', 'I think it is B, but it could very well be C. B is her being torn between home and the larger world.'),
    ('ENG-DIAG-T4-M2-Q11', 'C', 59, 2, 'misread_question', 'Read past "distinct". Thought the question asked what proves the two belong to the Peruvian marinera, not what proves they differ. Found D immediately once she reread the stem.', 'I thought the question asked what proves these are Peruvian styles. I did not read distinct.'),
    ('ENG-DIAG-T4-M2-Q12', 'B', 64, 3, 'solid_reasoning', 'Went straight for the option with the numbers and the link to people alive today. Exactly the right instinct on a strengthen question.', 'B has the facts, the numbers and "alive today", so it connects them directly.'),
    ('ENG-DIAG-T4-M2-Q13', 'D', 103, 3, 'solid_reasoning', 'Slowest question of the session and worth every second. Separated inference from fact by herself: A and B are facts, so they cannot be what the passage suggests.', 'Not B, that is a fact, not an inference. A is also a fact. C — terrestrial phenomena were never discussed.'),
    ('ENG-DIAG-T4-M2-Q14', 'C', 84, 3, 'solid_reasoning', 'Saw that "because" was doing the work and that the reason had to benefit the groupers, not describe the sharks.', 'B is irrelevant — you are still dead either way. They would only travel if it benefits them, which is the tides.'),
    ('ENG-DIAG-T4-M2-Q15', 'A', 39, 3, 'solid_reasoning', 'Rejected C as too obvious to be an inference — that is the right test on this question type.', 'C is a no-brainer, anyone can infer it, so it is not an implication. D — Earth is never mentioned.'),
    ('ENG-DIAG-T4-M2-Q18', 'A', 39, 1, 'concept_gap', 'Went to A on sound rather than rule. Once we did dependent versus independent clauses she got to C and could say why. This is the pattern across the grammar questions — the ear is good, the rules are not there yet.', 'Not B. It is A. (After the rule: it has to be C, the punctuation is proper and the dash pair is equal.)'),
    ('ENG-DIAG-T4-M2-Q20', 'A', 46, 3, 'solid_reasoning', 'Correct, and for the right reason — a transition in the middle of a sentence takes commas both sides, and the title cannot be split from the name.', 'Not B, not D. Deborah Gordon, however — reads both ways, but it is A. C would be a run-on.'),
    ('ENG-DIAG-T4-M2-Q21', 'B', 14, 3, 'solid_reasoning', 'Fastest answer of the session and right. Tense consistency spotted instantly; the past-perfect distinction we then worked through was new to her.', 'Were printed as a book and used. B.'),
    ('ENG-DIAG-T4-M2-Q22', 'C', 82, 3, 'solid_reasoning', 'Rejected A because it explains nothing, and got that the colon is doing explanatory work. Strong reasoning on punctuation she could not have named a rule for.', 'A is one continuous thing and does not explain. C is the colon with the explanation.'),
    ('ENG-DIAG-T4-M2-Q23', 'D', 35, 3, 'solid_reasoning', 'Saw that B and C imply a contradiction that the passage does not contain. Then versus now is a time contrast, and "Today" carries it.', 'B and C are contradicting. Meanwhile does the same. Today just agrees with the statement.')
    ) as t(source_ref, answered, elapsed, confidence, diagnosis, teacher_note, student_reasoning)
  loop
    n := n + 1;
    ...
    select v_session, q.id, v_student, n, 'revealed',
           v_clock, v_clock, v_clock + (r.elapsed || ' seconds')::interval,
           v_clock + (r.elapsed + 45 || ' seconds')::interval,
    ...
    -- Discussion ran roughly as long as the answer itself in the recording.
    v_clock := v_clock + (r.elapsed + 110 || ' seconds')::interval;
  end loop;
